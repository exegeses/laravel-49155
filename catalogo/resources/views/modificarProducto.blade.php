...
@section('contenido')
        ...
        <div class="alert bg-light border border-white shadow round col-8 mx-auto p-4">
            ...
            @foreach( $marcas as $marca )
                    <option {{ ( $marca->idMarca == old('idMarca', $Producto->idMarca) )?'selected':'' }} value="{{ $marca->idMarca }}">{{ $marca->mkNombre }}</option>
            @endforeach
                </select>
                <br>
                Categoría: <br>
            ...
            @foreach( $categorias as $categoria )
                    <option {{ ( $categoria->idCategoria == old('idCategoria', $Producto->idCategoria) )?'selected':'' }} value="{{ $categoria->idCategoria }}">{{ $categoria->catNombre }}</option>
            @endforeach
                </select>
                <br>
                Presentacion: <br>
